...
export default async function page() {
  ...
  const fetchAndUpdateData = async () => {
    if (FB_DB && id) {
      try {
        const docRef = doc(FB_DB, 'youtube', id); // Adjust 'youtube' to your collection name
  ...
  const video = await fetchAndUpdateData();
  ...
  const readableTimeStamp = video?.timestamp?.seconds
    ? formatTimestamp(video.timestamp.seconds)
    : 'Unknown';
      ...
      <div className="w-full min-h-screen h-full flex space-x-3">
        <div className="w-full xl:w-[80%] min-h-screen px-2">
          ...
          <div className="my-4 px-3">
            ...
            <p>
              Visit AIWeb3 official discord, #bot-channel, and enter the following if you want to contribute 10 points
              (keep in mind the ratio, you will earn more AIWEB token from the latest video)): <b>!tipCreator {id} 10</b>
            </p>
            <FullVideoStats
